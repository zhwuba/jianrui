...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mHomeOnDrawable = res.getDrawable(R.drawable.home_page_on);
        mHomeOffDrawable = res.getDrawable(R.drawable.home_page_off);
        mHomeDrawableWidth = res.getDimensionPixelSize(R.dimen.home_icon_width);
        mHomeDrawableHeight = res.getDimensionPixelSize(R.dimen.home_icon_height);
        mHomeDrawableOffsetLeft = res.getDimensionPixelSize(R.dimen.home_icon_offset_left);
        mHomeDrawableOffsetTop = res.getDimensionPixelSize(R.dimen.home_icon_offset_top);
        //*/
    }
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        /*/Modified by tyd Greg 2014-03-06,for show the name of shortcut which in hotseat
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        int x = (int) ev.getX();
        int y = (int) ev.getY();
        if ((getDrawHome()) && (checkHomePressed(x, y))) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        //*/Modified by tyd Greg 2014-04-03,for set default page
        if (this.mHomeOnDrawable != null) {
            int left = getWidth() - mHomeDrawableWidth - mHomeDrawableOffsetLeft;
            int top = mHomeDrawableOffsetTop;
            mHomeRect.set(left, top, left + mHomeDrawableWidth, top + mHomeDrawableHeight);
        }
        //*/
        int offset = getMeasuredWidth() - getPaddingLeft() - getPaddingRight() -
                (mCountX * mCellWidth);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    //*/Added by tyd Greg 2014-03-13
    
    //for edit mode
    public void enterEditMode() {
        mShortcutsAndWidgets.enterEditMode();
    }

    public void exitEditMode() {
        mShortcutsAndWidgets.exitEditMode();
    }
    
    //*/zhangwuba add 2014-5-6
     public void shakeOnceTime(){
    	 mShortcutsAndWidgets.shakeOnceTime();
     }
    //*/
    
    //for folder and apphub
    ...
    private boolean mNotShowForeground = false;

    void setOverScrollAmount(float r) {
        if (r >= 0.5f) {
            r = 1 - r;
        }

        mNotShowForeground = true;
        //mNormalBackground = getResources().getDrawable(R.drawable.panel_bg);
    ...
    private boolean mDrawHome = false;
    private boolean mPressedHomeIcon = false;
    private Rect mHomeRect = new Rect();
    private int mHomeDrawableWidth = -1;
    private int mHomeDrawableHeight = -1;
    private int mHomeDrawableOffsetLeft = -1;
    private int mHomeDrawableOffsetTop = -1;
    
    private Drawable mHomeOnDrawable = null;
    private Drawable mHomeOffDrawable = null;
        ...
        case MotionEvent.ACTION_DOWN:
            if(getDrawHome()){
                if (checkHomePressed(x, y)) {
                    mPressedHomeIcon = true;
                    invalidate(mHomeRect);
                    cancelLongPress();
                }
            }
            break;
        case MotionEvent.ACTION_UP:
            if (getDrawHome() && mPressedHomeIcon) {
                if(mLauncher != null){
                    mLauncher.setDefaultPageToWorkspace(this);
                }
                intercept = true;      
            }
            mPressedHomeIcon = false;
            invalidate(mHomeRect);
            break;
    ...
    public void setIsDefaultPage(boolean isDefault){
        mIsDefaultPage = isDefault;
        invalidate(mHomeRect);
    }
    ...
    public void setHomeOnDrawable(Drawable homeOn) {
        mHomeOnDrawable = homeOn;
    }

    public void setHomeOffDrawable(Drawable homeOff) {
        mHomeOffDrawable = homeOff;
    }

    private void drawHomeDrawable(Canvas canvas) {
        if (mIsDefaultPage) {
            mHomeOnDrawable.setBounds(mHomeRect);
            mHomeOnDrawable.draw(canvas);
        } else {
            mHomeOffDrawable.setBounds(mHomeRect);
            mHomeOffDrawable.draw(canvas);
        }
    }
    
    private boolean checkHomePressed(int x, int y) {
        return (x >= mHomeRect.left) && (x <= mHomeRect.right) && (y >= mHomeRect.top) && (y <= mHomeRect.bottom);
    }
    ...
    public void setDrawHome(boolean draw) {
        mDrawHome = draw;
    }
    //*/
}
