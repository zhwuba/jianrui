...
public final class Utilities {
    ...
    private static int mCellDelIconHeight = -1;
    private static int mCellDelIconWidth = -1;
    ...
    static int getUnistallBitmapWidth(Context context) {
        if (-0x1 == mCellDelIconWidth) {
            initUnistallBitmap(context);
        }
    ...
    static int getUnistallBitmapHeight(Context context) {
        if (-0x1 == mCellDelIconHeight) {
            initUnistallBitmap(context);
        }
        return mCellDelIconHeight;
    }
    
    static int getUnistallBitmapTop(Context context) {
        return  context.getResources().getDimensionPixelOffset(R.dimen.uninstall_app_toppadding);
    }
    
    static int getUnistallBitmapLeft(Context context) {
        return  context.getResources().getDimensionPixelOffset(R.dimen.uninstall_app_leftpadding);
    }
    
    private static void initUnistallBitmap(Context context) {
        Resources resource = context.getResources();
        Drawable uninstallIcon = resource.getDrawable(R.drawable.app_remove_btn);
        mCellDelIconWidth = resource.getDimensionPixelOffset(R.dimen.uninstall_app_icon_width);
        mCellDelIconHeight = resource.getDimensionPixelOffset(R.dimen.uninstall_app_icon_height);
        if (uninstallIcon instanceof BitmapDrawable) {
            BitmapDrawable bitmapDrawable = (BitmapDrawable) uninstallIcon;
            Bitmap uninstallBitmapTemp = bitmapDrawable.getBitmap();
            mUninstallBitmap = Bitmap.createScaledBitmap(uninstallBitmapTemp, mCellDelIconWidth, mCellDelIconHeight,
                    true);
            if ((uninstallBitmapTemp != null) && (!uninstallBitmapTemp.isRecycled())) {
                //uninstallBitmapTemp.recycle();
            }
        } else {
            Bitmap.Config config = (uninstallIcon.getOpacity() != -1) ? Bitmap.Config.ARGB_8888 : Bitmap.Config.RGB_565;
            mUninstallBitmap = Bitmap.createBitmap(mCellDelIconWidth, mCellDelIconHeight, config);
            sCanvas.setBitmap(mUninstallBitmap);
            uninstallIcon.draw(sCanvas);
    ...
    private static int mNewInstallPrefixHeight;
    private static int mNewInstallPrefixWidth;
    ...
    private static void initNewInstallPrefix(Context context) {
        Resources resource = context.getResources();
        mNewInstallPrefix = resource.getDrawable(R.drawable.new_install_prefix);
