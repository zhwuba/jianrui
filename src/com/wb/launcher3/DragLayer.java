...
public class DragLayer extends FrameLayout implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected boolean fitSystemWindows(Rect insets) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (child instanceof Insettable) {
                ...
                /*/Remaked by tyd Greg 2014-03-17,for apphub
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        //*/zhangwuba live weather 2014-8-18
        if (mLauncher != null) {
            mLauncher.updateMotionEvent(ev.getX(), ev.getY(), ev.getAction());
        }
        //*/
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            /*/Modified by tyd Greg 2013-11-11,for move shortcuts and appwidgets more quickly and convenient
            if (handleTouchDown(ev, false)) {
                return true;
            }
            //*/
            if (TydtechConfig.CAN_SNAP_PAGE_WHEN_DRAG) {
                handleTouchDown(ev, true);
            }else {
                if (handleTouchDown(ev, false)) {
                    return true;
                }
    ...
    //*/zhangwuba 2014-8-22
    private ViewDraw mViewDraw;
    
    public void setViewDrawFunc(ViewDraw draw){
		mViewDraw = draw;
	}
	
	public void draw(Canvas canvas){
		super.draw(canvas);
		//Log.i("myl","zhangwuba --------- MyImageView draw mViewDraw = " + mViewDraw);
		if(mViewDraw != null){
			mViewDraw.viewDraw(canvas);
		}
		
	}
    
    public abstract interface ViewDraw{
		public abstract boolean viewDraw(Canvas canvas); 
	}
    //*/
}
