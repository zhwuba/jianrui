...
interface OnTouchSwipeGestureListener{
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
	...
	private float mOldX,mOldY,mNewX,mNewY,tempX,tempY;//zxa brightness
    ...
    protected final static int TOUCH_SWIPE_DOWN_GESTURE = 5;
    protected final static int TOUCH_SWIPE_UP_GESTURE = 6;
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mContext = context;//added by zxa 1018 for adjust screen brightness
    ...
    protected void onAttachedToWindow() {
        ...
        if (mPageIndicator == null && mPageIndicatorViewId > -1) {
            ...
            mPageIndicator.addMarkers(markers, mAllowPagedViewAnimations, isWorkspace());//*/zhangwuba modify 2014-6-6
    ...
    public void scrollTo(int x, int y) {
        ...
        if (TydtechConfig.CYCLE_ROLL_PAGES_ENABLED) {
            ...
            int minScrollX = 0;
            int maxScrollX = mMaxScrollX;

            if (getChildCount() > 1) {
                minScrollX = -pageW;
                maxScrollX = mMaxScrollX + pageW ;
            }

            if (x < minScrollX) {
                super.scrollTo(minScrollX, y);
    ...
    public void onChildViewAdded(View parent, View child) {
        ...
        if (mPageIndicator != null && !isReordering(false)) {
            ...
            mPageIndicator.addMarker(pageIndex,
                    ...
                    mAllowPagedViewAnimations,isWorkspace());//*/zhangwuba modify 2014-6-6
        }
    ...
    private Intent mCloseBrightness = new Intent();
    public void setBrightness(int brightness) {  
        int tmpInt = Settings.System.getInt(getContext().getContentResolver(), 
                Settings.System.SCREEN_BRIGHTNESS, 255); 
        tmpInt = tmpInt + brightness;
        if (tmpInt > 255) {  
            tmpInt = 255;
	...
	public void closebrightnessDialog(){
			mCloseBrightness.setAction("tyd.android.intent.action.close_brightness_dialog");
              	getContext().sendBroadcast(mCloseBrightness);
	}

  public void adjustBrightness(MotionEvent ev,int pointCount){
                if(pointCount == 2){
                		
                		 mNewX = ev.getX();
                		 mNewY = ev.getY();
                		// Log.i("adjustBrightness", "mOldY="+mOldY+",tempY="+tempY);
                		// Log.i("adjustBrightness", "mNewX=="+mNewX+",mNewY=="+mNewY);
                		// Log.i("adjustBrightness", "Math.abs(mOldY - tempY)=="+Math.abs(mOldY - tempY)); 
                		if (Math.abs(mOldY - mNewY) > 0  && Math.abs(mNewX - mOldX) < Math.abs(mNewY - mOldY)) {  
    						if(tempY!=0){
    							setBrightness((int)(tempY - mNewY ) / 6);
    							Log.i("adjustBrightness", "---------------------1");
    						}else{
    							setBrightness((int)(mNewY - mOldY) / 6); 
    							Log.i("adjustBrightness", "---------------------2");
    					} 
    ...
    protected void snapToDestination() {
        //*/Added by tyd Greg 2014-03-20,for transition effect
        if (TydtechConfig.CYCLE_ROLL_PAGES_ENABLED && cycleSanpToPage()) {
            return;
        }
        //*/
    ...
    public void scrollLeft() {
        ...
        //*/Added by tyd Greg 2014-03-20,for transition effect
        else if (TydtechConfig.CYCLE_ROLL_PAGES_ENABLED && getNextPage() == 0) {
    ...
    public void scrollRight() {
        ...
        //*/Added by tyd Greg 2014-03-20,for transition effect
        else if (TydtechConfig.CYCLE_ROLL_PAGES_ENABLED && getNextPage() < getChildCount() - 1) {
            snapToPage(getChildCount() - 1);
        }
        //*/
    }
        ...
        if (mContentIsRefreshable) {
            ...
            if (page < count) {
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    //*/Added by tyd Greg 2014-03-20,for transition effect
                    if (TydtechConfig.CYCLE_ROLL_PAGES_ENABLED) {
                        if (mCurrentPage == count - 1 && i == 0
                                || mCurrentPage == 0 && i == count - 1) {
                            continue;
                        }
                    }
                    //*/
    ...
    //*/Added by tyd Greg 2014-03-20,for move shortcuts and appwidgets more quickly and convenient
    ...
    //for touch swipe gesture
    private OnTouchSwipeGestureListener mOnTouchSwipeGestureListener;
    
    public void setOnTouchSwipeGestureListener(OnTouchSwipeGestureListener listener) {
        mOnTouchSwipeGestureListener = listener;
    }
    
    public int mPreviousPage = INVALID_PAGE;

    //for transition effect and cycle scroll
    private int CYCLE_ADAPTER_DISTANCE = 0;
    
    private int m_moveNextDeltaX = 0;
    private boolean m_isSignificantMoveNext = false;
    protected boolean m_isScrollWallpaper = false;
    private boolean m_isCycleSnaped = false;
    protected int m_startScrollX;
    private boolean m_isRefreshed = false;
    
    public int getLastScreenCenter() {
        return mLastScreenCenter;
    }
    
	  
    private double mLastMultiDownDistance;
    
	///* zhangwuba add 2013-12-11
    private int x1Down = 0;
    private int x2Down = 0;
    private int y1Down = 0;
    private int y2Down = 0;
    private int xyD = 0;
    private int temp =0;
    //*/
    ...
    private double getDistance(MotionEvent ev) {
        float x = ev.getX(0) - ev.getX(1);
        float y = ev.getY(0) - ev.getY(1);
        return Math.sqrt(x * x + y * y);
    }
	
    protected void getVisiblePagesExt(int[] range , boolean isScrollToLeft) {
        final int pageCount = getChildCount();
        final int width = getViewportWidth();
        if (pageCount > 0 && width > 0) {
            int pageWidth = getScrollForPage(0);
            int leftScreen = 0;
            int rightScreen = 0;

            if (getScrollX() < 0) {
                leftScreen = rightScreen = 0;
            } else if (getScrollX() > mMaxScrollX) {
                leftScreen = rightScreen = pageCount - 1;
            } else {
                ...
                leftScreen = distance / width;
                rightScreen = Math.min(leftScreen + 1, pageCount - 1);
            }
    ...
    protected boolean cycleSanpToPage() {
        int totalDistance = getViewportWidth();
        int lastPage = getChildCount() - 1;
        int duration = PAGE_SNAP_ANIMATION_DURATION;
        boolean result = true;
        int delta = 0;

        if (mOverScrollX < 0 && m_moveNextDeltaX > 0 && m_isSignificantMoveNext) {
            delta = -totalDistance - mUnboundedScrollX;
            mNextPage = lastPage;
        } else if (mOverScrollX > mMaxScrollX && m_moveNextDeltaX < 0 && m_isSignificantMoveNext) {
            delta = mMaxScrollX + totalDistance - mUnboundedScrollX;
            mNextPage = 0;
        } else {
            result = false;
        }
        ...
        if (result) {
            ...
            m_isCycleSnaped = m_isScrollWallpaper = true;
            m_startScrollX = mUnboundedScrollX;
        }
